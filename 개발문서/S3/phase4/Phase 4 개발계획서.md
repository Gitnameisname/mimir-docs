...
| FG | 제목 | 진행 조건 |
|----|------|---------|
| **4-5** | Capability manifest 확장 (`default_enabled`/`requires`/`preferred_use`/`policy_profile`/`streaming_supported`) | 1라운드 R1~R5 회귀 녹색. 사용자 승인 시 즉시 진행. |
| **4-6** | L2 draft 쓰기 도구 실험적 노출 | idempotency·human approval·impact preview·감사 로그 4종 모두 갖춰진 후에만. **L3/L4는 본 Phase 범위에서 영구 제외.** |
...
- **`citations.citation_basis`** (Alembic revision `s3_p4_citation_basis`) — `VARCHAR(20) NOT NULL DEFAULT 'node_content'`. CHECK constraint: `citation_basis IN ('node_content', 'rendered_text')`. 기존 행은 default backfill.
...
- **`scope_profiles.allowed_tools`** (Alembic revision `s3_p4_scope_profile_allowed_tools`, FG 4-0 §2.1.6) — `JSONB NOT NULL DEFAULT '[]'::jsonb`. 기존 행 backfill: 빈 배열 (default-deny). 운영자가 Admin UI 로 도구 등록 전까지 에이전트 도구 호출 모두 거부 — **운영 절차 의존**. S3 Phase 3 정적 리뷰 P1 흡수 (task3-3.md §[129,223–225,318]).
...
- [ ] **R1 (L4 차단)** — `risk_tier=L4` 도구가 `tools/list`에 노출되지 않고, `tools/call`이 거부됨을 입증하는 회귀 테스트 녹색
- [ ] **R2 (ACL 단일)** — 신규 도구 3종이 모두 Scope Profile ACL을 통과한 후 결과를 반환함을 보장하는 회귀 테스트 녹색
- [ ] **R3 (pinned)** — `verify_citation`이 `latest`를 거부하고, REST에서 인용 생성 시 `vN`으로 resolve된 값만 저장됨을 입증하는 테스트 녹색
- [ ] **R4 (instruction_authority=none)** — read 응답 envelope이 항상 `instruction_authority: none`을 포함함을 입증하는 단위 테스트 녹색
- [ ] **R5 (REST 1:1 복제 금지)** — 신규 도구 3종이 REST 엔드포인트의 단순 wrapper가 아님을 검수보고서에서 함수-호출 그래프로 입증
- [ ] Alembic revision `s3_p4_citation_basis` 적용 + backfill 완료
- [ ] **Alembic revision `s3_p4_scope_profile_allowed_tools` 적용 + 운영자 Admin UI 도구 등록 완료** (FG 4-0 §2.1.6 — S3 P3 정적 리뷰 P1 흡수)
- [ ] **모든 MCP `tool_*` 진입점에 `_check_tool_allowed` 게이트 적용 + 회귀 ≥ 6 시나리오 녹색**
- [ ] **`종결회고.md §4.1 #11` / `FG3-3 §5 #9` 잔존부채 항목 종결 마크**
- [ ] `TOOL_SCHEMAS` 모든 항목에 `risk_tier`/`maturity`/`status` 부착
- [ ] Contract drift 4종 테스트 CI 게이트 등록
- [ ] 각 FG의 검수·보안 보고서 제출
...
- **Human Approval**: `@최철균` — 다음 시점에 의무
  ...
  - **Alembic revision `s3_p4_scope_profile_allowed_tools` 적용 직전** (FG 4-0 §2.1.6 — default-deny 라 운영자 도구 등록 작업과 같은 운영 창에 배치 필수)
  - 1라운드 종결 선언 직전
  - FG 4-5/4-6 진입 결정 시점
