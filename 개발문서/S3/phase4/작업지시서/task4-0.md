...
##### a. DB 스키마 변경 (Alembic revision `s3_p4_scope_profile_allowed_tools`)
...
1. **DB 마이그레이션 작성** — Alembic revision `s3_p4_scope_profile_allowed_tools` (§2.1.6.a). `--sql` dry-run 검증 후 운영자 승인.
...
- Alembic revision `s3_p4_scope_profile_allowed_tools` — `scope_profiles.allowed_tools` 컬럼 (`JSONB NOT NULL DEFAULT '[]'::jsonb`)
...
- [ ] Alembic revision `s3_p4_scope_profile_allowed_tools` 작성 + dry-run 검증 + 운영자 승인 + 적용
