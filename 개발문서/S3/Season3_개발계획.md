...
S3 이월 항목은 [`docs/개발문서/S3/이월.md`](이월.md) (등록부, 2026-05-18 신설) 가
정본이다. 본 등록부는 다음을 통합 관리한다:

- **Phase 6 이월 7항 (O1~O7)** — 2026-05-18 공식 종결 시점 등록.
  - O1 / O5 는 Phase 7 흡수 검토 (FG 7-4 / FG 7-6).
  - O2 / O3 / O4 는 별 라운드.
  - O6 은 별 ADR (다중 org admin UX).
  - O7 은 운영자 결정 (audit emit 실패 정책).
- Phase 4 이월 후보 등 향후 Phase 종결 시 동일 형식으로 추가.

S3 1라운드 범위에 포함되지 않은 항목을 "사실상 포함"시키지 않도록 한다 (범위
크립 방지). 새 항목 추가 시:
1. 출처 산출물 (검수보고서 / 종결보고서) 의 cross-reference 유지.
2. 분류 (Phase 흡수 / 별 라운드 / 별 ADR / 운영자 결정 / S3 2라운드 / S4) 명시.
3. 의존 (선행 인프라 / 정책 결정) + 승인 등급 (standard / P1) 명시.
