...
#### 목적
S2에서 새롭게 추가된 AI 기능들이 LLM 특화 보안 위험에 노출되지 않도록, OWASP Top 10 for LLM Applications의 모든 항목을 점검하고 대응책을 입증한다.
...
2. **OWASP Top 10 for LLM Applications 전 항목 점검**
   ...
   **LLM01: Prompt Injection**
   ...
   - 대응책:
     - **컨텐츠-지시 분리**: Phase 5 FG5.2에서 구현. 검색 결과에 `trusted: false` 메타 플래그 + delimiter
     - **입력 검증**: 검색 쿼리 길이 제한, 특수문자 샌드박싱
     - **프롬프트 구조화**: 체인-오브-스로트(chain-of-thought) 프롬프트로 논리 강화
     - **규칙 기반 탐지**: "이것은 untrusted 데이터입니다" 패턴 문자열 탐지 (룰 기반)
   - 검증 방법:
     ```python
