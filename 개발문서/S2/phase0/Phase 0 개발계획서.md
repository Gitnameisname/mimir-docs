...
4. `/api/v1/system/capabilities` 엔드포인트 신설 — **2-tier 정보 분리**

   **4a. Public Health 엔드포인트** (인증 불필요)
   - 경로: `/api/v1/system/health` (또는 `/.well-known/mimir-health`)
   - 노출 정보 (최소한):
   ...
   **4b. Capabilities 엔드포인트** (인증 필요 — 최소 authenticated user)
   - 경로: `/api/v1/system/capabilities`
   - 노출 정보:
     - `rag_available`: boolean (RAG 서비스 가용 여부)
     - `chunking_enabled`: boolean (문서 chunking 활성 여부)
   - 인증 필요: 로그인된 사용자 이상

   **4c. Admin Capabilities 엔드포인트** (Admin 권한 필요)
   - 경로: `/api/v1/admin/system/capabilities`
   - 추가 노출 정보:
   ...
   > **보안 근거**: provider 상세, pgvector 활성 여부 등 내부 구성 정보는 공격 표면 분석에 악용될 수 있으므로, 비인증 사용자에게 노출하지 않음
