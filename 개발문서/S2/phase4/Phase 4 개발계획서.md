...
Phase 3에서 완료하지 못하고 이월된 항목. Phase 4 작업과 병행하여 처리한다.

| ID | 항목 | 원래 Phase | 우선순위 | 처리 방침 |
|----|------|-----------|---------|-----------|
| PH3-CARRY-001 | **PII 배치 잡** — `conversations.expires_at` 기반 자동 만료·삭제 | Phase 3 | 높음 | Phase 4 인프라 구축(배포·스케줄러) 시 함께 구현. APScheduler 또는 Celery Beat 사용 |
| PH3-CARRY-002 | **대화 전문 FTS (tsvector)** — 제목 LIKE 검색 → 전문 Full-Text Search 전환 | Phase 3 | 낮음 | Phase 6(검색 고도화)으로 재이월. Phase 4에서는 LIKE 검색 유지 |

> **PH3-CARRY-001 상세**: Phase 3 `conversations` 테이블에 `expires_at` 컬럼이 추가되어 있으나, 만료된 레코드를 자동으로 삭제하는 배치 잡이 없다. Phase 4 배포 체계 구성 시 함께 구현하여 GDPR/개인정보 처리방침 준수를 확보한다.
