...
5. 작업 디렉토리의 **`functions.index.md`** — 구현·수정 전에 항상 먼저
...
- **부 책임**: 도구 계약 구현, 에러 계약 적용, functions.index.md 갱신, Change Boundary Block 작성.
...
4. **사전 탐색** — 해당 디렉토리의 `functions.index.md`를 먼저 읽는다(제11조). **이미 존재하는 함수를 재발명하지 않는다.**
...
7. **인덱스 동시 갱신** — 같은 커밋에서 `functions.index.md`를 갱신한다(제12조). 누락은 PR·CI에서 차단된다.
...
- ❌ `functions.index.md` 동시 갱신 없이 public/tool/effect/io/policy 함수 추가·변경 (제12조)
...
- 설계 결정 기록 → `docs/adr/`
- 모듈별 함수 지도 → 각 디렉토리의 `functions.index.md`
...
| 상황 | 참조 조항 |
| --- | --- |
| 함수를 새로 쓰기 전 | 제8~11 (품질 기준 + functions.index.md) |
