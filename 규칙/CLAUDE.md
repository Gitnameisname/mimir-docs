...
5. 작업 디렉토리의 **`functions.index.md`** — 함수 탐색 시
...
4. **컨텍스트 수집 순서** (제16조 trust level 준수):
   - `AGENT_MODE.md` → `project_policy` → `functions.index.md` → 관련 모듈 코드 → 관련 테스트 → ADR → 외부 자료(마지막).
...
4. **functions.index.md 동기화** (제12조) — public/tool/effect/io/policy 함수의 추가·변경이 인덱스에 반영되었는가.
...
- **functions.index.md를 먼저 읽는다**(제11조). 같은 기능이 이미 있는데 재발명하지 말 것.
...
2. functions.index.md를 **변경과 같은 커밋**에서 갱신한다(제12조).
...
- ❌ functions.index.md 갱신 없이 public/tool/effect/io 함수 변경 (제12조)
...
- 설계 결정 기록 → `docs/adr/`
- 함수 지도 → 각 디렉토리의 `functions.index.md`
